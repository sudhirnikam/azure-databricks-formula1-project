...
# MAGIC MANAGED LOCATION "abfss://demo@formula1dls7.dfs.core.windows.net"

# COMMAND ----------

results_df = spark.read \
.option("inferSchema", True) \
.json("abfss://raw@formula1dls7.dfs.core.windows.net/2021-03-28/results.json")
...
results_df.write.format("delta").mode("overwrite").save("abfss://demo@formula1dls7.dfs.core.windows.net/results_external")
...
# MAGIC USING DELTA
# MAGIC LOCATION 'abfss://demo@formula1dls7.dfs.core.windows.net/results_external'
...
results_external_df = spark.read.format("delta").load("abfss://demo@formula1dls7.dfs.core.windows.net/results_external")
...
# MAGIC WHERE try_cast(position as int) <= 10
...
deltaTable = DeltaTable.forName(spark, "f1_demo.results_managed")
...
.option("inferSchema", True) \
.json("abfss://raw@formula1dls7.dfs.core.windows.net/2021-03-28/drivers.json") \
.filter("driverId <= 10") \
...
# MAGIC %sql
# MAGIC SELECT * FROM f1_demo.drivers_merge TIMESTAMP AS OF '2026-02-22T13:40:46.000+0000';

# COMMAND ----------

df = spark.read.format("delta").option("timestampAsOf", '2026-06-22T15:40:33.000+0000').load("/mnt/formula1dl/demo/drivers_merge")
...
# MAGIC %sql
# MAGIC DROP TABLE f1_demo.drivers_convert_to_delta;
...
# MAGIC )
# MAGIC USING DELTA
# MAGIC LOCATION 'abfss://demo@formula1dls7.dfs.core.windows.net/drivers_convert_to_delta'
# MAGIC
# MAGIC -- CREATE TABLE f1_demo.results_external
# MAGIC -- USING DELTA
# MAGIC -- LOCATION 'abfss://demo@formula1dls7.dfs.core.windows.net/results_external'
...
df.write.format("parquet").save("abfss://demo@formula1dls7.dfs.core.windows.net/drivers_convert_to_delta_new")

# COMMAND ----------

# MAGIC %sql
# MAGIC CONVERT TO DELTA parquet.`abfss://demo@formula1dls7.dfs.core.windows.net/drivers_convert_to_delta_new`
